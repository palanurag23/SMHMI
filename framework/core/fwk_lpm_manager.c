...
static int _FWK_LpmManager_PreEnterSleepTimerCallback(lpm_dev_t *dev)
{
    ...
    {
        pDev->ops->stopPreEnterTimer(pDev);
...
static int _FWK_LpmManager_PreEnterSleep(lpm_dev_t *dev)
{
    ...
    if (s_LpmManager.currentMode == kLPMMode_SNVS)
    {
...
static int FWK_LpmManager_TimerCallback(lpm_dev_t *dev)
{
    ...
    if (s_LpmManager.enable == kLPMManagerStatus_SleepEnable)
    {
        ...
        {
            if ((pDev != NULL) && (pDev->ops != NULL) && (pDev->ops->enterSleep != NULL))
            {
...
int FWK_LpmManager_Start(int taskPriority)
{
    ...
    {
        LOGD("[LpmManager] START lpm dev [%d]", pDev->id);
        ret = s_LpmManager.dev->ops->openTimer(pDev);

        if (ret)
        {
            LOGE("[LpmManager] START lpm dev [%d] error: %d", pDev->id, ret);
        }
    }

    s_LpmManager.enable = kLPMManagerStatus_SleepEnable;
...
int FWK_LpmManager_RegisterRequestHandler(hal_lpm_request_t *req)
{
    ...
    lpm_request_item_t *pRequestItem = FWK_MALLOC(sizeof(lpm_request_item_t));
...
int FWK_LpmManager_UnregisterRequestHandler(hal_lpm_request_t *req)
{
    ...
    {
        LOGE("Failed to unregister handler for lpm request [%s]", req->name);
...
int FWK_LpmManager_RuntimeGet(hal_lpm_request_t *req)
{
    ...
    if ((pRequestItem == NULL) || (pxListItem == pxListEnd))
    {
        LOGE("Failed to \"get\" lpm runtime dev: %s", req->name);
...
int FWK_LpmManager_RuntimeSet(hal_lpm_request_t *req, int8_t count)
{
    ...
    if ((pRequestItem == NULL) || (pxListItem == pxListEnd))
    {
        LOGE("Failed to \"set\" lpm runtime dev: %s", req->name);
...
int FWK_LpmManager_RequestStatus(unsigned int *totalUsageCount)
{
    ...
    *totalUsageCount = 0;
    ...
    while (pxListItem != pxListEnd)
    {
        ...
        if ((pRequestItem != NULL) && (pRequestItem->dev != NULL))
        {
            LOGD("[LpmManager]: dev_name[%s], usage[%d]", pRequestItem->requestName, pRequestItem->usageCount);
...
int FWK_LpmManager_EnableSleepMode(hal_lpm_manager_status_t enable)
{
    ...
    if (s_LpmManager.enable != enable)
    {
        if (enable)
        {
            // enable the lpm
            if ((pDev != NULL) && (pDev->ops != NULL) && (pDev->ops->openTimer != NULL))
            {
                LOGD("[lpm_manager] START lpm dev [%d]", pDev->id);
                ret = s_LpmManager.dev->ops->openTimer(pDev);

                if (ret)
                {
                    LOGE("[lpm_manager] START lpm dev [%d] error: %d", pDev->id, ret);
                }
                else
                {
        ...
        {
            // disable the lpm
            ret = s_LpmManager.dev->ops->stopTimer(pDev);

            if (ret)
            {
                LOGE("STOP lpm dev [%d] timer error: %d", pDev->id, ret);
            }

            ret = s_LpmManager.dev->ops->stopPreEnterTimer(pDev);

            if (ret)
            {
                LOGE("STOP lpm dev [%d] PreEnterTimer error: %d", pDev->id, ret);
            }
