...
const static flash_dev_t *s_FlashDev = NULL;
...
{
    sln_flash_status_t ret = kStatus_HAL_FlashFail;
    if ((s_FlashDev != NULL) && (s_FlashDev->ops->format != NULL))
    {
...
sln_flash_status_t FWK_Flash_Read(const char *path, void *buf, unsigned int offset, unsigned int *size)
{
    ...
    {
        ret = s_FlashDev->ops->read(s_FlashDev, path, buf, offset, size);
    }
    return ret;
}
...
{
    sln_flash_status_t ret = kStatus_HAL_FlashFail;
    if ((s_FlashDev != NULL) && (s_FlashDev->ops->mkfile != NULL))
    {
        ret = s_FlashDev->ops->mkfile(s_FlashDev, path, encrypt);
    }
    return ret;
}
