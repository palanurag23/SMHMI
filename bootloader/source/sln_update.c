...
status_t SLN_Update_MoveApp(fica_img_type_t newAppImg)
{
    ...
    if (kStatus_Success == status)
    {
        configPRINTF(("[SLN_UPDATE] Start moving app img %d.\r\n"));
...
status_t SLN_Update_ParseBundle(fica_img_bank_t bundleBank)
{
    ...
    if (kStatus_Success == status)
    {
        ...
        flashError        = FICA_GetFreeImgSlot(&newAppImg);
        FICA_SetCurAppStartType(newAppImg);
    }
...
status_t SLN_Update_VerifyImgBank(fica_img_type_t imgType, uint32_t *imgSig)
{
    /* Used to write the signature of the images */
}
