...
static uint8_t BOARD_ConfigMPUNonCached(uint8_t regionStartingNumber, uint32_t regionStartingAddress, uint32_t size)
{
    ...
    while ((numberOfRegions < availableRegions) && (size != 0))
    {
        ...
        MPU->RASR = ARM_MPU_RASR(0, ARM_MPU_AP_FULL, 2, 0, 0, 0, 0, power - 1);
...
void BOARD_ConfigMPU(void)
{
    ...
    uint8_t region = BOARD_ConfigMPUNonCached(0, nonCacheStart, nonCacheSize);

#if defined(__USE_SHMEM)
    region = BOARD_ConfigMPUNonCached(region, rpmsgShmemStart, rpmsgShmemSize);
#endif

    /* NCACHE_REGION_CM7 */
    region = BOARD_ConfigMPUNonCached(region, nonCacheStartCM7, nonCacheSizeCM7);
