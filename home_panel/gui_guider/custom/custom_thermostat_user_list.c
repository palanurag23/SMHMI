...
static const char *s_ModeStr[kLanguage_Ids][kThermostat_NumMode] = {
	{
		"Auto",
		"Cool",
		"Heat",
...
static const char *s_FanSpeedStr[kLanguage_Ids][kThermostat_NumFanSpeed] = {
	{
		"Slow",
		"Middle",
		"Fast",
...
void gui_thermostat_set_user_info(thermostat_user_list_obj_t *user_info, uint8_t item_no)
{
	if (item_no <= s_UserItemNum)
	{
		...
		strcpy(&s_UserInfo[item_no].user_name, user_info->user_name);
		strcpy(&s_UserInfo[item_no].temperature, user_info->temperature);
...
void gui_thermostat_lvgl_refresh_user_list(uint8_t item_no)
{
	...
	memcpy(s_UserItemStr, &s_UserInfo[item_no].user_name, strlen(&s_UserInfo[item_no].user_name));
	memcpy(&s_UserItemStr[TEMPERATURE_STR_LOC], &s_UserInfo[item_no].temperature, strlen(&s_UserInfo[item_no].temperature));
	memcpy(&s_UserItemStr[MODE_STR_LOC], s_ModeStr[language][s_UserInfo[item_no].mode], strlen(s_ModeStr[language][s_UserInfo[item_no].mode]));
	memcpy(&s_UserItemStr[FAN_SPEED_STR_LOC], s_FanSpeedStr[language][s_UserInfo[item_no].fan_speed], strlen(s_FanSpeedStr[language][s_UserInfo[item_no].fan_speed]));
...
void gui_thermostat_lvgl_edit_user_item(void)
{
	...
	char *user_name = lv_textarea_get_text(guider_ui.screen_thermostat_user_edit_ta_user_name);
	strcpy(&user_info.user_name, user_name);
	lv_dropdown_get_selected_str(guider_ui.screen_thermostat_user_edit_ddlist_temperatrue, &user_info.temperature, TEMPERATURE_VALUE_SIZE);
...
void gui_thermostat_lvgl_refresh_user_editing_item(void)
{
	...
	user_info = gui_thermostat_get_user_info(s_UserItemSelectedNo);

	lv_textarea_set_text(guider_ui.screen_thermostat_user_edit_ta_user_name, &user_info.user_name);
	int opt = lv_dropdown_get_option_index(guider_ui.screen_thermostat_user_edit_ddlist_temperatrue, &user_info.temperature);
