...
typedef struct _respond_add_thermostat_user_event
{
    uint16_t id;
...
typedef struct _security_user_list_obj
{
    uint16_t face_id;
    uint8_t  user_name[FACE_NAME_MAX_LEN];
} security_user_list_obj;

typedef struct _security_users_share_db
{
    uint32_t count;
    security_user_list_obj objs[MAX_FACE_DB_SIZE];
